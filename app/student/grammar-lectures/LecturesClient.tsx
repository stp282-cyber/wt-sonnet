...
interface LecturesClientProps {
    initialBooks: GrammarBook[];
}

export default function LecturesClient({ initialBooks }: LecturesClientProps) {
    const searchParams = useSearchParams();
    const targetBookId = searchParams.get('bookId');

    const [books, setBooks] = useState<GrammarBook[]>(initialBooks);
    const [selectedSection, setSelectedSection] = useState<GrammarSection | null>(null);

    // Automatically load content if coming from Quick Menu (URL has bookId)
    useEffect(() => {
    ...
    }, [targetBookId]); // Removed 'books' from dependency to prevent loop, initialBooks handles first load
