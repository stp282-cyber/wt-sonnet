...
export default function ReviewTestPage() {
    ...
    const [words] = useState<Word[]>([
        ...
        { no: 9, english: 'cherry', korean: '체리' },
        { no: 10, english: 'lemon', korean: '레몬' },
    ]);
    ...
    const generateChoices = (correctWord: Word, allWords: Word[]): string[] => {
        const choices: string[] = [correctWord.korean];
        ...
        for (let i = 0; i < 4 && i < shuffled.length; i++) {
            choices.push(shuffled[i].korean);
        }
    ...
    // 문제 생성
    useEffect(() => {
        const generatedQuestions: MultipleChoiceQuestion[] = words.map((word) => {
            const choices = generateChoices(word, words);
            const correctIndex = choices.indexOf(word.korean);
                    ...
                    <Group mb={20} justify="space-between">
                        ...
                        >
                            <Group gap="xs">
                                ...
                                <Text fw={900} size="xl" c={timeLeft <= 3 ? 'red' : 'violet'}>
                                    {timeLeft}초
                                </Text>
                            </Group>
                        </Paper>
